    ...
    public function getByCourseId(int $courseId): array
    {
        ...
        $result = json_decode(json_encode(
            DB::select(
                "SELECT $selectColumns
                FROM {$this->tableName}
                JOIN students_courses_enrollments
                ON students_courses_enrollments.courses_id = :courseId
                WHERE students.id = students_courses_enrollments.students_id AND students_courses_enrollments.deleted_at IS NULL",
                [
                    'courseId' => $courseId
                ]
            )
