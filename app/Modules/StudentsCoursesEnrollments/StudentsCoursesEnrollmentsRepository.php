...
class StudentsCoursesEnrollmentsRepository
{
    ...
    public function update(StudentsCoursesEnrollments $studentsCoursesEnrollments): StudentsCoursesEnrollments
    {
        return DB::transaction(function () use ($studentsCoursesEnrollments) {
            DB::table($this->tableName)->updateOrInsert([
                'id' => $studentsCoursesEnrollments->getId()
            ], $studentsCoursesEnrollments->toSql());

            $id = ($studentsCoursesEnrollments->getId() === null || $studentsCoursesEnrollments->getId() === 0)
                ? (int)DB::getPdo()->lastInsertId()
                : $studentsCoursesEnrollments->getId();
